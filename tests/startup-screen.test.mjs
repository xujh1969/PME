...
const styles = readdirSync(stylesDir)
  ...
  .join("\n");
const variablesStyles = readFileSync(new URL("variables.css", stylesDir), "utf8");
...
test("keeps an empty themed startup surface before the app bundle renders", () => {
  ...
  assert.equal(/:root\s*\{[\s\S]*?color:\s*var\(--color-ink\);[\s\S]*?background:\s*var\(--color-canvas\);/.test(variablesStyles), true);
